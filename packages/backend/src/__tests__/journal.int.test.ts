...
let token: string;
let userId: string;
const TEST_EMAIL = `journal-test+${Date.now()}@local`;
const TEST_PASS = "journaltestpass123";

beforeAll(async () => {
  if (!process.env.DATABASE_URL)
    throw new Error("DATABASE_URL must be set for integration tests");

  // Register a new test user for journal tests
  const registerRes = await request(app)
    .post("/api/auth/register")
    .send({ email: TEST_EMAIL, password: TEST_PASS });

  if (!registerRes.body.token || !registerRes.body.user?.id) {
    throw new Error("Failed to register test user for journal tests");
  }
